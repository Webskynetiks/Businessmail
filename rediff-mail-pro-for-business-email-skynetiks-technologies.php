<?php include_once "common/header.php" ?>
<style type="text/css">
    .main-header {
        position: inherit;
    }

    .pricingTable10 .month {
        font-size: 27px;
        color: #fff;
        position: absolute;
        /* bottom: 15px; */
        right: -40px !important;
        /* text-transform: uppercase; */
    }

    .link-solution {
        color: #015790 !important;
        font-weight: 600 !important;
    }
</style>


<section class="city" style="background:linear-gradient(rgb(33 109 161 / 81%), rgb(1 87 146 / 33%)),url(images/rediff-background.png.png);background-repeat: no-repeat;
    background-size: 100% 100%;">
    <div class="container">
        <div class="text-center city_banner">
            <h4>Rediff Mail Pro - The most economical & affordable email solution.</h4>
            <div class="btns-box">
                <a href="#" class="theme-btn btn-style-one"><span class="txt" data-toggle="modal" data-target="#callback">Get a Call Back</span></a>
            </div>
        </div>

    </div>
</section>

<section class="google_workspace_content">
    <div class="container">
        <div class="sec-title centered">
            <h2>Rediffmail Pro <span>in India</span> </h2>
        </div>
        <div class="row">
            <div class="col-md-8">

                <div class="text-content">
                    <p><strong>Rediffmail Pro is a business email solution which provides 1GB of storage with every email address. It is designed to cater all the needs of small enterprise-scale businesses.</strong> Rediffmail Pro provides all the features for live collaboration, chats and email communications between multiple users.
                    </p>
                    <p>
                        Rediffmail offers fast and secure webmail services for your business domain, along with a wide range of features to enhance collaboration and productivity.
                    </p>
                    	<h5><span style="color:#0c82dc;"><b><a href="https://www.microsoft.com/en-in/microsoft-365/microsoft-365-for-existing-subscribers">Our Customers benefits:</a></span></b></h5>
					
					 <p style="color:#000;">
		<b>1. Rapid response time</br>
2. Experienced and top consultants </br>
3. 24/7 Support </br>
4. Telephonic, Video Call and Screen Sharing Support
</b>
                </div>
            </div>
            <div class="col-md-4">
                <img src="images/how-do-I-setup-rediff-mail-pro-with-my-existing-domain-skynetiks-technologies-an-official-Google-Workspace-partner-in-Noida-India.png" title="Is domain registration included in the Rediffmail Pro email hosting package - Skynetiks Technologies is a complete solution." alt="How do I buy or renew a Rediffmail Pro subscription online with skynetiks technologies an official Google Workspace partner in Noida, India">
            </div>
        </div>
    </div>

</section>

<section class="pricing_table">
    <div class="container">
        <div class="sec-title centered">
            <h2>Rediffmail Pro <span>Pricing and Plans</span> </h2>
        </div>
        <div class="row">
            <div class="col-md-4 ">
                <div class="pricingTable10">
                    <div class="pricingTable-header">
                        <h3 class="heading">Basic</h3>
                        <span class="price-value">
                            <span class="currency"> ₹</span>270
                            <span class="month">/Yr</span>
                        </span>
                    </div>
                    <div class="pricing-content">
                        <ul>
                            <li>1 GB Storage Space</li>
                            <li>1 GB Storage Space</li>
                            <li>1Spam & Virus Protection</li>
                            <li>1Data Localization</li>
                            <li>1Common Login Page</li>
                        </ul>
                        <span ><a href="signup-google-workspace-microsoft-O365-zoho-workplace-30days-moneyback-guarantee-skynetiks-technologies-partner-Noida-Gurugram-India.php#query-form-signup" class="read">sign up</a></span>
                    </div>
                </div>
            </div>
            <div class="col-md-4 ">
                <div class="pricingTable10">
                    <div class="pricingTable-header">
                        <h3 class="heading">Most Popular</h3>
                        <span class="price-value">
                            <span class="currency">₹ </span> 510
                            <span class="month">/Yr</span>
                        </span>
                    </div>
                    <div class="pricing-content">
                        <ul>
                            <li>10 GB Storage Space</li>
                            <li>Spam & Virus Protection</li>
                            <li>Data Localization</li>
                            <li>Common Login Page</li>
                            -
                        </ul>
                        <span ><a href="signup-google-workspace-microsoft-O365-zoho-workplace-30days-moneyback-guarantee-skynetiks-technologies-partner-Noida-Gurugram-India.php#query-form-signup" class="read">sign up</a></span>
                    </div>
                </div>
            </div>
            <div class="col-md-4 ">
                <div class="pricingTable10">
                    <div class="pricingTable-header">
                        <h3 class="heading">Advanced</h3>
                        <span class="price-value">
                            <span class="currency">₹</span> 850
                            <span class="month">/Yr</span>
                        </span>
                    </div>
                    <div class="pricing-content">
                        <ul>
                            <strong>Everything in Most Popular +</strong>
                            <li>Custom Branding</li>
                            <li>* Rcloud</li>
                            <li>* DMARC</li>
                        </ul>
                        <span ><a href="signup-google-workspace-microsoft-O365-zoho-workplace-30days-moneyback-guarantee-skynetiks-technologies-partner-Noida-Gurugram-India.php#query-form-signup" class="read">sign up</a></span>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>





<?php include_once "common/service_network.php" ?>
<?php include_once "common/footer.php" ?>